...
Item {
  ...
  // Process has no environment property. info (+ mDNS chatter at warn)
  // keeps the transfer protocol visible in the shell log via the helper's
  // stderr without per-frame debug noise; raise rqs_lib to debug when
  // troubleshooting a transfer.
  readonly property var helperCommand: [
    "/usr/bin/env",
    "RUST_LOG=info,mdns_sd=warn",
    helperPath,
    destinationDir !== "" ? "--out" : "",
